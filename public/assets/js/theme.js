var theme = (function (t, $) {
  ...
  t.product = {
      ...
      var found = children.find(function (variant) {
        ...
        selectedAttributes.forEach(function (selectedAttribute) {
          for (let i = 0; i < variant.variant_attribute_values.length; i++) {
            if (
              variant.variant_attribute_values[i].code ===
              selectedAttribute.name
            ) {
              isMatch.push(
                variant.variant_attribute_values[i].id ===
                  parseInt(selectedAttribute.value)
              );
              break;
            }
          }
    ...
    updatePrice: function (variant) {
      ...
      $(`[${t.product.selector.salePrice}]`).html(
        variant.purchase_info.price.format
      );
      $(`[${t.product.selector.retailPrice}]`).html(
        variant.purchase_info.price_retail.format
      );
    },
    updateForm: function (variant) {
      ...
      if (
        !variant ||
        variant.purchase_info.availability === 'outofstock' ||
        variant.purchase_info.availability === 'unavailable'
      ) {
      ...
      var addProductUrl = $(selector)
        .attr('action')
        .replace(/\d+/g, variant.id);
    ...
    updateImage: function (variant) {
      ...
      var slideIndex = $(t.product.selector.sliderNav)
        .find('#' + slideno)
        .data('slick-index');
    ...
    handleSubscriptionOption: function () {
      $(
        'input[name=' + t.product.selector.subscriptionOptionInputName + ']'
      ).click(function () {
      ...
      $('#product-subscribe-options-select').on(
        'click',
        'input',
        t.product.setOptionToHiddenField
      );
    },
    ...
    setIntervalValue: function () {
      $('#product-subscribe-options-select input[type=radio]:first').prop(
        'checked',
        true
      );
      $('#id_interval').val(
        $("#product-subscribe-options-select input[type='radio']")
          .first()
          .attr('interval')
      );
      $('#id_interval_count').val(
        $("#product-subscribe-options-select input[type='radio']").first().val()
      );
    },
    clearIntervalValue: function () {
      $('#product-subscribe-options-select input[type=radio]').prop(
        'checked',
        false
      );
